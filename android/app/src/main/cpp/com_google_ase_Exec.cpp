...
static int create_subprocess(
  char* const* args, char* const* env, int* pProcessId) {
  char devname[32];
  int ptm;
  pid_t pid;
...
JNIEXPORT jobject JNICALL Java_com_google_ase_Exec_createSubprocess(
    JNIEnv* env, jclass clazz, jobjectArray args, jobjectArray envVars, jintArray processIdArray) {
  int argsCount = env->GetArrayLength(args);
  char** args_8 = (char**)malloc((argsCount + 1) * sizeof(char*));
  if (args_8 == 0) {
    JNU_ThrowByName(env, "java/lang/OutOfMemoryError", 0);
    return 0;
  }
  for (int i = 0; i < argsCount; i++) {
    args_8[i] = JNU_GetStringNativeChars(env, (jstring)env->GetObjectArrayElement(args, i));
    if (args_8[i] == 0) {
      return 0;
    }
  }
  ...
  char** env_8 = (char**)malloc((envCount + 1) * sizeof(char*));
  if (env_8 == 0) {
    JNU_ThrowByName(env, "java/lang/OutOfMemoryError", 0);
    return 0;
  }
  for (int i = 0; i < envCount; i++) {
    env_8[i] = JNU_GetStringNativeChars(env, (jstring)env->GetObjectArrayElement(envVars, i));
    if (env_8[i] == 0) {
      return 0;
    }
  }
  ...
  int procId;
  int ptm = create_subprocess(args_8, env_8, &procId);
